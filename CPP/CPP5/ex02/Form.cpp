...
Form::Form(std::string name, unsigned int gradeToSign, unsigned int gradeToExecute) : _formName(name), _gradeToSign(gradeToSign), _gradeToExecute(gradeToExecute), _signed(false)
{
...
{	
	if (A.getGrade() < getFormGradeToSign())
		_signed = true;
	else
		throw Form::FormGradeTooLow();
}
